...
var Search = /*#__PURE__*/_react.default.memo(function (_ref6) {
  ...
  (0, _react.useEffect)(function () {
    var focusSearch = function focusSearch(event) {
      ...
      if (!inputRef.current || inputRef.current === _global.document.activeElement) return;

      if ( // Shift is required to type `/` on some keyboard layouts
      (0, _keybinding.matchesModifiers)({
        ctrl: false,
        alt: false,
        meta: false
      }, event) && (0, _keybinding.matchesKeyCode)('Slash', event)) {
    ...
    _global.document.addEventListener('keyup', focusSearch);

    return function () {
      return _global.document.removeEventListener('keyup', focusSearch);
    };
