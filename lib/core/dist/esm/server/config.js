...
var loadConfig = async function (options) {
  ...
  var presetsConfig = [...corePresets, require.resolve('./common/babel-cache-preset'), ...frameworkPresets, ...loadCustomPresets(options), ...overridePresets]; // Remove `@storybook/preset-typescript` and add a warning if in use.
  ...
  var presets = loadPresets(filteredPresetConfig, restOptions);
